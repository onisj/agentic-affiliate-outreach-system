...
class MessageResponse(Base, BaseModel):
    """Model for storing responses to messages."""
    __tablename__ = "message_responses"
    
    message_id = Column(UUID(as_uuid=True), ForeignKey("message_logs.id", ondelete="CASCADE"), nullable=False)
    response_type = Column(String(50), nullable=False)  # e.g., "open", "click", "reply"
    platform = Column(String(50), nullable=False)  # e.g., "email", "linkedin", "twitter"
    response_metadata = Column(JSONB, default={}, server_default='{}', nullable=False)
    timestamp = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    
    # Relationships
    message = relationship("MessageLog", back_populates="responses")
    
    __table_args__ = (
        Index('ix_message_responses_message_id', 'message_id'),
        Index('ix_message_responses_response_type', 'response_type'),
        Index('ix_message_responses_timestamp', 'timestamp'),
    )